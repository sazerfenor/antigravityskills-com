...
// Schema tables will be loaded dynamically based on database provider
let permission: any;
let role: any;
...
async function loadSchema() {
  const isSqlite = ['sqlite', 'turso'].includes(envConfigs.database_provider);
  const schema = isSqlite
    ? await import('@/config/db/schema.sqlite')
...
async function initializeRBAC() {
  ...
  // Load schema based on database provider
  await loadSchema();
  ...
  try {
      ...
      for (const permCode of roleData.permissions) {
        ...
        if (permCode.endsWith('.*')) {
          ...
          for (const permId of matchingPerms) {
            await (db() as any).insert(rolePermission).values({
              id: getUuid(),
              roleId,
              permissionId: permId,
            });
    ...
    if (adminEmailArg) {
      ...
      if (adminUser) {
        ...
        const [existingUserRole] = await (db() as any)
          .select()
          .from(userRole)
          .where(
        ...
        if (!existingUserRole) {
          await (db() as any).insert(userRole).values({
            id: getUuid(),
            userId: adminUser.id,
