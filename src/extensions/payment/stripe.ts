  ...
  private mapStripeEventType(eventType: string): PaymentEventType {
    switch (eventType) {
      ...
      case 'charge.refund.updated':
        console.log(`[Stripe] Refund event detected: ${eventType}`);
        return PaymentEventType.PAYMENT_REFUNDED;
      // 🚨 争议/Chargeback 事件 - 精确处理
      // 注意：Stripe 的 dispute.closed 需要检查 status 字段来判断输赢
      // 这里我们只标记为 REFUNDED，让 notify 路由根据具体情况处理
      case 'charge.dispute.funds_withdrawn':
        // 银行已从商家账户扣款 = 用户拿到钱 = 必须回收积分
        console.log(`[Stripe] Dispute funds withdrawn - Credits will be revoked: ${eventType}`);
        return PaymentEventType.PAYMENT_REFUNDED;
      case 'charge.dispute.created':
        // 争议开始，Stripe 会暂扣资金，但结果未定
        // 保守策略：先回收积分，如果后续商家赢了可以手动恢复
        console.log(`[Stripe] Dispute CREATED - Preemptively revoking credits: ${eventType}`);
        return PaymentEventType.PAYMENT_REFUNDED;
      case 'charge.dispute.closed':
        // 争议关闭，可能是 won/lost/warning_closed 等
        // 需要在 notify 路由中检查具体 status
        console.log(`[Stripe] Dispute CLOSED - Checking status to determine action: ${eventType}`);
        return PaymentEventType.PAYMENT_REFUNDED;
      // 🚨 Stripe Radar 早期欺诈预警 - 强烈建议主动退款
      case 'radar.early_fraud_warning.created':
        console.warn(`[Stripe] ⚠️ RADAR FRAUD WARNING - Proactive refund recommended: ${eventType}`);
        return PaymentEventType.FRAUD_WARNING;
      default:
