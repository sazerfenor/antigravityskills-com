  ...
  private mapCreemEventType(eventType: string): PaymentEventType {
    switch (eventType) {
      ...
      case 'refund.completed':
        console.log(`[Creem] Refund event detected: ${eventType}`);
        return PaymentEventType.PAYMENT_REFUNDED;
      // 🚨 争议/Chargeback 事件 - 保守止损策略
      case 'dispute.lost':
        // 用户胜诉 = 用户拿到退款 = 确认回收积分
        console.log(`[Creem] Dispute LOST (user won) - Credits will be revoked: ${eventType}`);
        return PaymentEventType.PAYMENT_REFUNDED;
      case 'dispute.created':
        // 争议开始 = 用户意图明确 = 立即回收积分（止损优先）
        // 如果商家最终胜诉，可通过管理后台手动恢复
        console.log(`[Creem] Dispute CREATED - Preemptively revoking credits: ${eventType}`);
        return PaymentEventType.PAYMENT_REFUNDED;
      case 'dispute.won':
        // 商家胜诉 = 用户钱不退 = 积分已在 created 时回收，无需再处理
        // 管理员可手动恢复积分
        console.log(`[Creem] Dispute WON (merchant won) - No further action (credits already revoked at creation)`);
        throw new Error(`Dispute won by merchant, manual credit restoration may be needed: ${eventType}`);
      default:
