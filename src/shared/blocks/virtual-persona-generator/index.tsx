 ...
 * 虚拟人格生成器组件
 *
 * @description 从 admin/virtual-personas 页面抽取的可复用组件
 *              用于批量生成虚拟人格，包含详情预览和灵魂验证
 */
        ...
        <CollapsibleContent className="mt-3 space-y-3">
          ...
          <div className="rounded-md bg-primary/10 p-3 border border-primary/20">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-medium">{persona.username} 的回复</span>
            </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="font-semibold text-base truncate">{persona.username}</h3>
            </div>
...
export function VirtualPersonaGenerator() {
    ...
    try {
      ...
      for (const item of result.data.results) {
        if (item.debug) {
          ...
          pushDebug('virtual-persona', `生成人格: ${item.persona.username}`, {
            personaName: item.persona.username,
      ...
      console.error('[VirtualPersonaGenerator] Generate error:', error);
    } finally {
  ...
  const handleGenerateAvatar = useCallback(async (index: number) => {
    ...
    // 记录开始日志
    pushDebug('avatar-generation', `开始生成头像: ${item.persona.username}`, {
      username: item.persona.username,
    ...
    try {
      ...
      const avatarUrl = await generateAndUploadAvatar(
        ...
        (step, progress) => {
          if (step === 'generating') {
            ...
            pushDebug('avatar-generation', `AI 生成中: ${item.persona.username}`, {
              username: item.persona.username,
              step: 'generating',
              progress,
            });
      ...
      // 记录成功日志
      pushDebug('avatar-generation', `头像生成成功: ${item.persona.username}`, {
        username: item.persona.username,
        avatarUrl,
        step: 'success',
      });

      toast.success(`${item.persona.username} 的头像生成完成`);
    } catch (error: any) {
      console.warn('[VirtualPersonaGenerator] Avatar generation failed:', error);

      // 记录失败日志（详细错误信息）
      pushDebug('avatar-generation', `❌ 头像生成失败: ${item.persona.username}`, {
        username: item.persona.username,
        avatarPrompt: item.persona.avatarPrompt,
        step: 'error',
  ...
  const handleSave = useCallback(async (index: number) => {
    ...
    try {
      ...
      toast.success(`${item.persona.username} 已保存`);
    } catch (error: any) {
