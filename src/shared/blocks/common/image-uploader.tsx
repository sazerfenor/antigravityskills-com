...
interface ImageUploaderProps {
  ...
  uploadType?: string; // avatar, ai-image-reference, temp
...
export const uploadImageFile = async (file: File, uploadType?: string) => {
  // Import compression library dynamically
  const imageCompression = (await import('browser-image-compression')).default;

  // Compression options based on upload type
  const getCompressionOptions = (type?: string) => {
    switch (type) {
      case 'avatar':
        return {
          maxSizeMB: 1,
  ...
  const compressionOptions = getCompressionOptions(uploadType);

  try {
    const compressed = await imageCompression(file, compressionOptions);
    console.log('[Compression] Original size:', formatBytes(file.size));
    console.log('[Compression] Compressed size:', formatBytes(compressed.size));
    console.log('[Compression] Upload type:', uploadType || 'temp');

    // 压缩后输出为 webp，需要更新文件名扩展名
    const baseName = file.name.replace(/\.[^.]+$/, '');
    const newFileName = `${baseName}.webp`;
    compressedFile = new File([compressed], newFileName, { type: 'image/webp' });
  } catch (error) {
...
export function ImageUploader({
  ...
  const replaceItems = (pairs: Array<{ id: string; file: File }>) => {
    pairs.forEach(({ id, file }) => {
      ...
      uploadImageFile(file, uploadType)
        .then((url) => {
          setItems((prev) =>
            prev.map((item) => {
              if (item.id !== id) return item;
  ...
  const handleFiles = (selectedFiles: File[]) => {
    ...
    Promise.all(
      newItems.map(async (item) => {
        try {
          const url = await uploadImageFile(item.file as File, uploadType);
          setItems((prev) => {
