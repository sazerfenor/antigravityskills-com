...
// ============================================================================
// Credit Building Helper
// ============================================================================

/**
 * Build a NewCredit object for order/subscription payments
 *
 * This helper consolidates the credit construction logic to avoid duplication
 * across handleCheckoutSuccess, handlePaymentSuccess, and handleSubscriptionRenewal.
 *
 * Note: This returns a NewCredit object to be used within a transaction,
 * NOT directly inserted. This preserves atomicity with order/subscription updates.
 */
const buildCreditForOrder = ({
  userId,
  userEmail,
  orderNo,
  subscriptionNo,
  ...
  if (!credits || credits <= 0) return undefined;

  const expiresAt = calculateCreditExpirationTime({
    creditsValidDays: creditsValidDays || 0,
    currentPeriodEnd,
  });
...
export async function handleCheckoutSuccess({
  ...
  // 如果订单已经是支付成功状态，直接返回，不再重复发分
  // 防止 Webhook 重放攻击或支付提供商超时重试导致重复发放积分
  if (order.status === OrderStatus.PAID) {
    console.warn(
      `[Payment] Idempotency check triggered: Order ${orderNo} is already paid. ` +
      `Ignoring duplicate webhook. TransactionId: ${session.paymentInfo?.transactionId || 'unknown'}`
    );
    return;
  }
  ...
  if (session.paymentStatus === PaymentStatus.SUCCESS) {
    ...
    const newCredit = buildCreditForOrder({
      userId: order.userId,
