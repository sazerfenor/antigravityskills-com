    ...
    <SidebarSection title={title} contentClassName="p-0">
      <div className="divide-y divide-border-medium/30">
        {items.map((item, index) => (
          ...
          >
            <summary className="text-sm font-medium text-foreground hover:text-primary hover:drop-shadow-[0_0_4px_var(--color-primary)] transition-colors py-3 cursor-pointer list-none flex items-center justify-between">
              <span className="text-left pr-2">{item.question}</span>
              <svg
                className="w-4 h-4 shrink-0 text-muted-foreground group-open:rotate-180 group-open:text-primary group-open:drop-shadow-[0_0_4px_var(--color-primary)] transition-transform" 
                fill="none" 
                viewBox="0 0 24 24" 
                stroke="currentColor"
              >
