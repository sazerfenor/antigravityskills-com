...
export async function requireAdminAccess({
  ...
  console.log('[RBAC] requireAdminAccess called');
  
  const user = await getSignUser();
  console.log('[RBAC] getSignUser result:', user ? { id: user.id, email: user.email } : 'null');

  if (!user) {
    console.log('[RBAC] No user found, redirecting to sign-in');
    redirect({ href: '/sign-in', locale: locale || '' });
  }

  console.log('[RBAC] Checking admin.access permission for user:', user!.id);
  const allowed = await canAccessAdmin(user!.id);
  console.log('[RBAC] admin.access permission result:', allowed);

  if (!allowed) {
    console.log('[RBAC] Permission denied, redirecting to:', redirectUrl);
    redirect({ href: redirectUrl || '', locale: locale || '' });
  }
