...
import { checkoutSchema } from '@/shared/schemas/api-schemas';
import { ErrorLogger } from '@/shared/lib/error-logger';
import { ErrorFeature } from '@/shared/models/error_report';

export async function POST(req: Request) {
  try {
    // ✅ Zod 参数校验 - 防止恶意数据注入（支付接口高危！）
    const validation = await validateRequest(req, checkoutSchema);
    if (!validation.success) {
      return validation.response;
    }
    
    const { product_id, currency, locale, payment_provider, metadata } = validation.data;
    
    // 🛡️ Rate Limit - 支付接口严格使用 IP 限制（防测卡攻击）
    const ip = getClientIP(req);
    const rateLimitResult = await checkRateLimit(
      `payment:checkout:ip:${ip}`,
      RATE_LIMITS.PAYMENT_CHECKOUT.limit,
      RATE_LIMITS.PAYMENT_CHECKOUT.window
    );
    if (!rateLimitResult.success) {
      return respErr('Too many payment attempts. Please wait a moment.');
    }
