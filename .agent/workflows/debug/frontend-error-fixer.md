...
### Step 2: 三层验证诊断 ⭐ V2.1 ENHANCED

> [!IMPORTANT]
> **不要假设代码正确就等于问题解决。必须完成三层验证。**
...
2. 定位具体文件和行号
3. 检查周围代码上下文
4. 查找最近变更: `git diff --name-only HEAD~5`

#### Layer 2: 网络层 (如涉及 HTTP/API)

> **触发条件**: 错误涉及 fetch/axios/API 调用/CORS/下载
...
5. **必须**执行以下验证之一:
   ...
   curl -I "{url}"
   ```
   - 或使用 `browser_subagent` 截图 DevTools Network 面板
   - 或要求用户提供 Network 截图

**GATE**: 如涉及网络但无法获取证据 → PAUSE，询问用户

#### Layer 3: 环境层 (如浏览器行为异常)
...
**完成标准**: 至少完成 Layer 1 + (Layer 2 或 Layer 3 中的一个，根据问题类型)
