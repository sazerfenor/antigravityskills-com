...
3. **Database & Data Layer Analysis** (MANDATORY for DB-related features)
   ...
   - Analyze ORM/ODM patterns (Prisma, TypeORM, Sequelize, Drizzle, SQLAlchemy, Django ORM, 
   ...
   - **Capture schema conventions**: naming (snake_case/camelCase), timestamp patterns, FK patterns
   - **Identify constraints**: UNIQUE, CHECK, FK ON DELETE behaviors in existing schema
...
### 3. Database & Data Layer (MANDATORY for DB-related features)

> [!IMPORTANT]
> This section is **required** when the feature involves database changes. Use the structured format below.

**Stack Summary Table**:

| Aspect | Current Value | File Path |
|--------|--------------|----------|
| Database Engine | [SQLite/PostgreSQL/None] | [env file or config] |
| ORM | [Drizzle/Prisma/None] | [schema file path] |
| Migration Tool | [Drizzle Kit/Prisma Migrate/Raw SQL] | [migration folder] |
| Type Generation | [drizzle-zod/prisma-client/none] | [generated types path] |

**Schema Conventions**:
- **Table Naming**: [snake_case/camelCase/PascalCase]
- **Column Naming**: [snake_case/camelCase]
- **Timestamp Pattern**: [`created_at`/`createdAt`] + [`updated_at`/`updatedAt`]
- **FK Column Pattern**: [`table_id`/`tableId`/`table_fk`]
- **Existing Constraints**: [UNIQUE on X, CHECK on Y, FK ON DELETE CASCADE/SET NULL]

**Existing Schema Files** (with line numbers):
- [schema file 1 path] - Tables: X, Y, Z
- [migration folder path] - N migration files

**Query Patterns**:
- Transaction handling: [explicit `.transaction()` / implicit]
- Connection pooling: [yes/no, pool size if applicable]
- Concurrency mode: [WAL/default]
