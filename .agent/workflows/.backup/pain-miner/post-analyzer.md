...
> ⛔ **CRITICAL**: 对 Step 1 返回的**每一个** Lead，必须执行 Step 2-4。**禁止只处理部分 Lead**。

**循环结构**:
```javascript
const allLeads = await mcp_sqlite_read_query("SELECT * FROM leads WHERE analyzed = 0");
...
for (const lead of allLeads) {
  ...
  await executeStep2_5_to_3(lead);
  await executeStep4_UpdateDB(lead);
}
```
...
**Version**: 2.0 | **Owner**: post_analyzer (Lead Analysis Only) | **Updated**: 2025-12-25
