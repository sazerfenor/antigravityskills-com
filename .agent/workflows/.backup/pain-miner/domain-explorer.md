...
查询 SQLite `complaint_keywords` 表获取高优先级抱怨词:
```sql
SELECT ck.keyword, cc.name as category, cc.priority
FROM complaint_keywords ck
JOIN complaint_categories cc ON ck.category_id = cc.id
WHERE cc.priority >= 4
ORDER BY cc.priority DESC, RANDOM()
LIMIT 6;  -- 获取 6 个候选词
```

**选择规则** (⚙️ 重要变更):
1. 每轮选择 **3-4 个** 抱怨词 (不是 1-2 个)
2. **多类别覆盖**: 必须包含至少 2 个不同类别 (如 emotional + delegation)
3. **轮换机制**: 优先选择本周未使用过的词
...
> 🔍 DECISION_GATE:
>   - 可用抱怨词总数: {N}
>   - 选择的抱怨词: {keyword1, keyword2, keyword3, keyword4}
>   - 类别覆盖: {category1, category2}
>   - Action: PROCEED  # ⛔ 禁止 SKIP
> ```
> **⛔ 禁止跳过**: 即使无可用抱怨词，也必须使用默认词继续执行。
> **默认词**: "nightmare", "frustrating", "hiring someone", "tedious"
