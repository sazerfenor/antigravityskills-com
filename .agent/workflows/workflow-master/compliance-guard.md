...
| **Prompt 质量** | 20% (2分) | Sub-Agent 指令是否有 Role/Input/Output/GATE? |
| **字符效率** | 10% (1分) | 是否 < 10,000 chars? 是否简洁无废话? |

### Prompt 质量检查细则 ⭐ V3.0 NEW

| 检查项 | 得分 | 检查方法 |
|--------|------|----------|
| **Skill 调用证据** | 0.5分 | Content Writer 是否报告读取了 Skill |
| **技术应用证据** | 0.5分 | 是否标注了使用的 Prompt 技术 |
| **自检清单完成** | 0.5分 | 自检清单是否全部勾选 |
| **无模糊表述** | 0.5分 | grep 搜索无 "可能/大概/建议" |

```bash
# 模糊表述检测
grep -E "(可能|大概|建议|或许)" [workflow_file]
# 结果为空 = ✅ 通过
# 结果非空 = ❌ 扣 0.5 分
```
...
| 检查项 | 状态 | 说明 |
|--------|------|------|
| 需要 Skill 的 Agent 是否有调用语法 | ✅/❌ | `> 参考 .agent/skills/xxx/SKILL.md` |
| 配套 Skill 是否已生成 | ✅/❌ | 检查 .agent/skills/ 目录 |
| Skill INPUT 是否匹配 Agent OUTPUT | ✅/❌ | 链条验证 |
